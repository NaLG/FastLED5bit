...
CLEDController &CFastLED::addLeds(CLEDController *pLed,
  ...
  CLEDController &CFastLED::addLeds(CLEDController *pLed,
    ...
    pLed->init();
    pLed->setLeds(data + nOffset, bdata + nOffset, nLeds);
    FastLED.setMaxRefreshRate(pLed->getMaxRefreshRate(),true);
    return *pLed;
  }
  ...
  CLEDController &CFastLED::addLeds(CLEDController *pLed,
    ...
    pLed->init();
    // CLEDController & setLeds(CRGB *data, CRGB *dataLo, uint8_t *bdata0, uint8_t *bdata1, uint8_t *bdata2, int nLeds) {
    pLed->setLeds(data + nOffset, dataLo + nOffset, bdata0 + nOffset, bdata1 + nOffset, bdata2 + nOffset, nLeds);
    FastLED.setMaxRefreshRate(pLed->getMaxRefreshRate(),true);
    return *pLed;
  }
#endif
...
void CFastLED::show(uint8_t scale) {
  ...
  while(pCur) {
    // Serial.write("pCur!\n");
    // Serial.write("gongetdith!\n");
    uint8_t d = pCur->getDither();
    if(m_nFPS < 100) { pCur->setDither(0); }
    // Serial.write("gonshowled!\n");
    pCur->showLeds(scale);
    // Serial.write("gonsetdith!\n");
    pCur->setDither(d);
    pCur = pCur->next();
  }
  // Serial.write("count!\n");
  countFPS();
  // Serial.write("finshow!\n");
}
